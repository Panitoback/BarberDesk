-- Adds visits.appointment_id FK so the dashboard can resolve the real charged
-- price (visits.price, set when completing with extras) instead of falling
-- back to appointments.price (the booking snapshot).
ALTER TABLE visits
...
BEGIN
  ...
  IF COALESCE(v_loyalty_active, true) THEN
    INSERT INTO visits (tenant_id, client_id, appointment_id, service, date, price, points_earned)
    VALUES (p_tenant_id, v_client_id, p_appointment_id, v_service, v_date, v_final_price, 10);
  ...
  INSERT INTO visits (tenant_id, client_id, appointment_id, service, date, price, points_earned)
  VALUES (p_tenant_id, v_client_id, p_appointment_id, v_service, v_date, v_final_price, 0);
