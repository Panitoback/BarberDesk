-- Adds a per-appointment price captured at booking time (looked up from
-- tenants.config.services in the booking API). complete_appointment now
-- carries that price into the resulting visit so dashboards can total
-- revenue from completed visits.

ALTER TABLE appointments
  ADD COLUMN IF NOT EXISTS price numeric;

CREATE OR REPLACE FUNCTION public.complete_appointment(p_appointment_id uuid, p_tenant_id uuid)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  IF COALESCE(v_loyalty_active, true) THEN
    ...
    VALUES (p_tenant_id, v_client_id, v_service, v_date, v_price, 10);
  ...
  INSERT INTO visits (tenant_id, client_id, service, date, price, points_earned)
  VALUES (p_tenant_id, v_client_id, v_service, v_date, v_price, 0);
