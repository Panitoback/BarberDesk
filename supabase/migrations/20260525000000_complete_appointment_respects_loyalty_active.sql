-- complete_appointment now checks automations_config.loyalty_active.
-- When loyalty is OFF, the appointment still completes and the visit is still
-- recorded, but no points/level update or loyalty_add log entry happens.
...
BEGIN
  ...
  INSERT INTO visits (tenant_id, client_id, service, date, points_earned)
  VALUES (p_tenant_id, v_client_id, v_service, v_date, 0);
